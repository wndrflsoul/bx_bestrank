...
if(WIZARD_FIRST_INSTAL !== "Y")
{
	...
	foreach ($arGroups as $arGroup)
	{
		...
		if ($arGroup["STRING_ID"] == "EMPLOYEES_".WIZARD_SITE_ID)
		{
			...
			{
				WizardServices::SetFilePermission(Array(WIZARD_SITE_ID, "/bitrix/"), Array($groupID => 'R'));
			}
			else
			{
		...
		if (WIZARD_IS_RERUN === false && !defined("ADDITIONAL_INSTALL"))
		{
			if ($arGroup["STRING_ID"] == "EMPLOYEES_".WIZARD_SITE_ID)
			{
	...
	if (!WIZARD_IS_RERUN === false)
	{
		...
		if($groupID && WIZARD_SITE_DEPARTAMENT){
			...
			if ($arIBlock = $rsIBlock->Fetch())
			{
				...
				$startDepartment = $departmentRepository->getById((int)WIZARD_SITE_DEPARTAMENT);
				$departmentCollection = $departmentRepository->getAllTree(
					$startDepartment,
					\Bitrix\Intranet\Enum\DepthLevel::FULL,
					\Bitrix\Intranet\Enum\DepartmentActiveFilter::ONLY_GLOBAL_ACTIVE
				);
				$arSectionUsers = [];
				foreach($departmentCollection as $department)
				{
					$arSectionUsers[] = $department->getId();
				}
